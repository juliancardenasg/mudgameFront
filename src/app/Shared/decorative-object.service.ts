...
export class DecorativeObjectService {
  private decorativeObjectLoad = new BehaviorSubject<Decorativeobject>(
    new Decorativeobject('', -1)
  );
  ...
  findAll(): Observable<Decorativeobject[]> {
    return this.http.get<Decorativeobject[]>(
      'http://localhost:8090/decorativeobject/all'
    );
  }

  findById(id: number): Observable<Decorativeobject> {
    return this.http.get<Decorativeobject>(
      'http://localhost:8080/decorativeobject/' + id + '/get'
    );
  }

  save(item: Decorativeobject): Observable<Decorativeobject> {
    return this.http.post<Decorativeobject>(
      'http://localhost:8080/decorativeobject/save',
      item
    );
  }
  ...
  delete(id: number): Observable<number> {
    return this.http.get<number>(
      'http://localhost:8080/decorativeobject/' + id + '/delete'
    );
  }
}
